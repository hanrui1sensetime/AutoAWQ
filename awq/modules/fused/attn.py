...
class RoPE(nn.Module):
    def __init__(self, hidden_size, n_heads, max_seq_len, device, base=10000):
        super(RoPE, self).__init__()
        self.base = base

        self.freqs_cis = nn.Parameter(
            self.precompute_freqs_cis(hidden_size // n_heads, max_seq_len * 2, theta=self.base).to(device),
            requires_grad=False
        )
...
class QuantAttentionFused(nn.Module):
    def __init__(self, hidden_size, n_heads, n_kv_heads, qkv_layer, o_proj, dev, max_seq_len,
        ...
        self.rope_theta = rope_theta  # for rope
        ...
        if use_alibi:
            ...
            self.alibi = None
            self.rope = RoPE(hidden_size, n_heads, max_seq_len, dev, base=self.rope_theta)
    ...
    def forward(self, hidden_states:torch.Tensor, attention_mask=None, *args, **kwargs):
        ...
        if seqlen > 1 or not FT_INSTALLED:
            ...
            if FLASH_INSTALLED and not self.use_alibi:
                ...
                output = flash_attn_func(xq.transpose(1, 2), keys.transpose(1, 2), values.transpose(1, 2), causal=True, window_size=(self.max_seq_len, self.max_seq_len))
                # print(output)
