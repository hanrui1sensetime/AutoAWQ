...
class AwqQuantizer:
    def __init__(self, awq_model, model, tokenizer, w_bit, group_size, version,
                       calib_data, split, text_column, batch_block) -> None:
        ...
        self.batch_block = batch_block  # for big patch of calib
        self.modules, self.module_kwargs, self.inps = self.init_quant(n_samples=512, seqlen=512)
    ...
    def _search_best_scale(self, module, prev_op, layers: List[nn.Linear], inp: torch.Tensor, module2inspect=None, kwargs={}):
        ...
        with torch.no_grad():
            ...
            for i in range(0, inp.shape[0], self.batch_block):
                if 'attention_mask' in kwargs:
                    fp16_output_batch = module2inspect(inp[i:i+self.batch_block].to(dev), attention_mask=kwargs['attention_mask'][i:i+self.batch_block].to(dev), position_ids=kwargs['position_ids'], past_key_value=kwargs['past_key_value'], output_attentions=kwargs['output_attentions'], padding_mask=kwargs['padding_mask'])
                else:
                    fp16_output_batch = module2inspect(inp[i:i+self.batch_block].to(dev))
        ...
        '''
        if 'attention_mask' in kwargs:
            kwargs['attention_mask'] = kwargs['attention_mask'].cuda()
        '''
        '''
    ...
    def _compute_best_scale(self, x, w_max, x_max, module2inspect, linears2scale: List[nn.Linear],
        ...
        for ratio in range(n_grid):
            ...
            for i in range(0, x.shape[0], self.batch_block):
                if 'attention_mask' in kwargs:
                    int_w_output_batch = module2inspect(x[i:i+self.batch_block].to(device), attention_mask=kwargs['attention_mask'][i:i+self.batch_block].to(device), position_ids=kwargs['position_ids'], past_key_value=kwargs['past_key_value'], output_attentions=kwargs['output_attentions'], padding_mask=kwargs['padding_mask'])
                else:
                    int_w_output_batch = module2inspect(x[i:i+self.batch_block].to(device))
    ...
    def init_quant(self, n_samples=128, seqlen=512):
        ...
        class Catcher(nn.Module):
            ...
            def forward(self, hijacked_inputs, **kwargs):
                inps.append(hijacked_inputs.to('cpu'))
                if 'attention_mask' in layer_kwargs.keys():
                    kwargs['attention_mask'] = torch.cat((layer_kwargs["attention_mask"].to('cpu'), kwargs['attention_mask'].to('cpu')), dim=0)
        ...
        for i in range(0, samples.shape[0], self.batch_block):
            try:
                self.model(samples[i:i+self.batch_block].to(next(self.model.parameters()).device))
        ...
        '''
        if "attention_mask" in layer_kwargs.keys():
            layer_kwargs["attention_mask"] = layer_kwargs["attention_mask"]
            # .to("cuda")

        clear_memory()
        '''
        # keep them on cpu for awq infinity batch
    ...
    def _get_input_feat(self, layer, named_linears):
        ...
        for i in range(0, self.inps.shape[0], self.batch_block):
            ...
            # self.inps[i:i+per_batch] = self.inps[i:i+per_batch].to(dev)  # in case multi-gpu
            if 'attention_mask' in self.module_kwargs:
                self.inps[i:i+self.batch_block] = layer(self.inps[i:i+self.batch_block].to(dev), attention_mask=self.module_kwargs['attention_mask'][i:i+self.batch_block].to(dev), position_ids=self.module_kwargs['position_ids'], past_key_value=self.module_kwargs['past_key_value'], output_attentions=self.module_kwargs['output_attentions'], padding_mask=self.module_kwargs['padding_mask'])[0].to('cpu')
            else:
                self.inps[i:i+self.batch_block] = layer(self.inps[i:i+self.batch_block].to(dev)).to('cpu')
            # self.inps = self.inps.to(next(layer.parameters()).device)  # in case multi-gpu
            # self.inps[i:i+per_batch] = layer(self.inps[i:i+per_batch], **self.module_kwargs)[0].to('cpu')
            clear_memory()
